...
Pr_sh = xlsread('Input.xlsx','Shared_En_Price');        % Shared Price
...
load Inputs_MAT_0 % load the values of P_L_dr_II_fixed_next and SoC next: Note that they can be []
% load Inputs_MAT
...
if SoC_next == -1                       % Initial value of the State of Charge
    ...
    E_0 = SoC_next;                   % Value derived from the previous software running
end
...
elseif xl(2,1)=="Lognormal"
    for t = 1:nt
        m(t)= P_load_input(t,1);  % Actual mean value
        s(t) = P_load_input(t,2);  % Actual standard deviation
        if m(t)==0
            P_load(t,:)=zeros(1,N);
        else
            P_load(t,:) = lognrnd(m(t), s(t), [1, N]);
        end
    end
    elseif xl(2,1)=="Weibull"
        for t = 1:nt
            % P_load_input(t,1) = shape parameter (k)
            % P_load_input(t,2) = scale parameter (lambda)
            k = P_load_input(t,1);      % shape
            lambda = P_load_input(t,2); % scale
            P_load(t,:) = wblrnd(lambda, k, [1, N]);
        end
    end
...
elseif xg(2,1) == "Lognormal"
    for t = 1:nt
        m(t)= P_gen_input(t,1);  % Actual mean value
        s(t) = P_gen_input(t,2);  % Actual standard deviation
        if m(t)==0
            P_gen(t,:)=zeros(1,N);
        else
            P_gen(t,:) = lognrnd(m(t), s(t), [1, N]);
        end
    end
elseif xg(2,1) == "Weibull"
    for t = 1:nt
        % P_gen_input(t,1) = shape parameter (k)
        % P_gen_input(t,2) = scale parameter (lambda)
        k = P_gen_input(t,1);      % shape
        lambda = P_gen_input(t,2); % scale
        if k==0
            P_gen(t,:)=zeros(1,N);
        else 
        P_gen(t,:) = wblrnd(lambda, k, [1, N]); 
        end
    end

elseif xg(2,1) == "Rayleigh"
    for t = 1:nt
        % P_gen_input(t,1) = scale parameter (k)
        k = P_gen_input(t,1);      % scale
        %         if k==0
        %             P_gen(t,:)=zeros(1,N);
        %         else
        P_gen(t,:) = raylrnd(k,[1, N]);
        %         end
    end
...
elseif xg(2,1)=="Beta"
        ...
        % Definisci i limiti dell'intervallo reale
        a = 0;   % ad esempio 0
        b = P_gen_input(1,3);   % ad esempio 100
        for t = 1:nt
            alpha=P_gen_input(t,1);
            beta=P_gen_input(t,2);
            if alpha==0
                P_gen(t,:) = zeros(1,N);
            else
            % --- GENERAZIONE CAMPIONI BETA SU [a, b] ---
            P_gen(t,:) = a + (b - a) * betarnd(alpha, beta, [1, N]);
             end
        end
end
...
for n_iter=1:N
    ...
    if exit_flag(n_iter)>0
        ...
        P_l = P_l+P_dr;
        E_shar(n_iter_aux,:) = min([P_l';P_gt']);
    end
end
...
E_shared_mean = mean(E_shar)*Pb;
E_shared_std = std(E_shar)*Pb;
SoC_mean = mean(SoC);
SoC_std = std(SoC);
...
TargetSheet = get(WorkSheets,'item','Shared_Energy');
Activate(TargetSheet);
Excel.Range('A3:D1002').Select;     % It is recomended to increase the number of rows in case of more than 1000 time intervals
Excel.Selection.Clear;
...
xlswrite('Output.xlsx',E_shared_mean','Shared_Energy','A3')
xlswrite('Output.xlsx',E_shared_std','Shared_Energy','B3')
